...
"""
        ...
        # Use ThreadPoolExecutor to avoid blocking the async loop
                        ...
                        if inPuTMsG.strip().startswith('/quick'):
                            ...
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /gh (team_code) [emote_id] [target_uid]\n\n[FFFFFF]Examples:\n[00FF00]/quick ABC123[FFFFFF] - Join, send Rings emote, leave\n[00FF00]/ghquick ABC123[FFFFFF] - Ghost join, send emote, leave\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/re'):
                            print('Processing reject spam command in any chat type')
    
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /re (target_uid)\nExample: /re 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                        ...
                        if inPuTMsG.startswith('/x'):
                            # Process /x command in any chat type
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /x (team_code)\nExample: /x ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
                        ...
                        if inPuTMsG.strip().startswith('/gh'):
                            # Process /gh command in any chat type
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /gh (team_code)\nExample: /gh ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
                        ...
                        if inPuTMsG.strip().startswith('/st'):
                            # Process /st command in any chat type
                            initial_message = f"[B][C]{get_random_color()}\nStarting match...\n"
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/sm_inv'):
                            print('Processing spam invite with cosmetics')
    
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ Usage: /sm_inv (uid)\nExample: /sm_inv 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
...
# IMPROVED TREE-STYLE HELP MENU SYSTEM (Commands in their original menus) 🌳
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = f"[b][c]{get_random_color()}Hey User Welcome To A W A N I S H ˣ ʙᴏᴛ"
...
[00FFFA]║
[FFFF00]║ 👤 Developer    :: [FF1493]NIROB
[32CD32]║ 💻 Developer        :: [32CD32]RABBI
...
async def MaiiiinE():
    Uid , Pw = '4296052072','EE69D0687C6D468EEFC1AD707BCCEF6C879A0885CF4BFD522A7733E67D947C3B'
...
def handle_keyboard_interrupt(signum, frame):
    ...
    print("👋 Thanks for using RAOSTAR_APIS")
    sys.exit(0)
